...
export const invoiceExceptionWorkflowPack = defineWorkflowPack({
  ...
  createGeneratedRuntimeArtifacts: createInvoiceGeneratedRuntimeArtifacts,
...
function createInvoiceGeneratedRuntimeArtifacts(specification: WorkflowSpecification) {
  const managerThreshold = managerThresholdFromSpecification(specification);
  return {
    agentSource: `# Generated by Tacit. Execution is validated by the restricted runtime.\n\nMANAGER_APPROVAL_THRESHOLD = ${managerThreshold}\n\n\ndef evaluate(payload):\n    purchase_order_quantity = payload.get('purchaseOrderQuantity')\n    if purchase_order_quantity in (None, 0):\n        return {'decision': 'request_more_information', 'reason': 'Purchase order is missing'}\n    if payload.get('duplicateInvoice'):\n        return {'decision': 'reject_or_escalate', 'reason': 'Possible duplicate'}\n    if payload.get('deliveryConfirmed') is False:\n        return {'decision': 'hold', 'reason': 'Delivery confirmation is missing'}\n    if payload.get('invoiceUnitPrice') != payload.get('purchaseOrderUnitPrice'):\n        return {'decision': 'escalate_to_procurement', 'reason': 'Price mismatch'}\n    variance = abs(payload['invoiceQuantity'] - purchase_order_quantity) / purchase_order_quantity\n    if variance > 0.02:\n        return {'decision': 'escalate', 'reason': 'Variance exceeds tolerance'}\n    if payload.get('emailApproval') == 'conditional':\n        return {'decision': 'human_review', 'reason': 'Email approval is conditional'}\n    if payload.get('invoiceValue', 0) > MANAGER_APPROVAL_THRESHOLD:\n        return {'decision': 'manager_approval', 'reason': 'Value exceeds manager threshold'}\n    return {'decision': 'approve', 'reason': 'Exact match or tolerance with delivery confirmation'}\n`,
    testSource: `from agent import MANAGER_APPROVAL_THRESHOLD, evaluate\n\n\ndef test_quantity_tolerance_requires_delivery_confirmation():\n    outcome = evaluate({'invoiceQuantity': 99, 'purchaseOrderQuantity': 100, 'invoiceUnitPrice': 100, 'purchaseOrderUnitPrice': 100, 'deliveryConfirmed': False, 'duplicateInvoice': False, 'emailApproval': 'none'})\n    assert outcome['decision'] == 'hold'\n\n\ndef test_high_value_invoice_requires_manager_approval():\n    outcome = evaluate({'invoiceQuantity': 100, 'purchaseOrderQuantity': 100, 'invoiceUnitPrice': 100, 'purchaseOrderUnitPrice': 100, 'deliveryConfirmed': True, 'invoiceValue': MANAGER_APPROVAL_THRESHOLD + 1, 'duplicateInvoice': False, 'emailApproval': 'none'})\n    assert outcome['decision'] == 'manager_approval'\n`,
  };
}

function managerThresholdFromSpecification(specification: WorkflowSpecification): number {
  const rule = specification.rules.find((item) => item.id === 'manager_threshold');
  const digits = rule?.condition.match(/\d[\d,]*/)?.[0]?.replace(/,/g, '');
  const threshold = digits ? Number(digits) : 500000;
  return Number.isFinite(threshold) && threshold > 0 ? threshold : 500000;
}
