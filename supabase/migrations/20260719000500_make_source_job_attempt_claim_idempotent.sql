-- A manually recovered job can reuse an attempt number after its retry budget
-- is reset. Update the ledger row instead of making the worker crash on a
-- uniqueness conflict before it can resume unfinished artifacts.
