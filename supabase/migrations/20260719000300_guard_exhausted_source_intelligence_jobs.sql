-- The platform-job constraint caps attempts at five. Do not select an exhausted
-- queued row, because incrementing it would make the claim RPC fail and restart
-- the worker instead of surfacing a terminal job failure.

update public.platform_jobs
set status = 'failed',
    completed_at = now(),
    updated_at = now(),
    error_message = coalesce(error_message, 'Source intelligence exhausted its retry limit.')
where kind in ('source_interpretation', 'cross_source_understanding')
  and status = 'queued'
  and attempts >= 5;
