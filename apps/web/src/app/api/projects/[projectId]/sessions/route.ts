...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  } catch (error) { const message = error instanceof Error ? error.message : 'Unable to save the observation.'; return Response.json({ error: message }, { status: error instanceof z.ZodError || error instanceof ObservationPersistenceError ? 400 : 500 }); }
}
