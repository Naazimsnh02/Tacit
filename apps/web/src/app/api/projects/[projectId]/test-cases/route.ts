...
import { genericCaseInputSchema, genericCaseOutcomeSchema } from '../../../../../lib/generic-process-pack';

/**
 * Historical cases are process-agnostic: any non-empty JSON input/outcome is
 * accepted. Evidence filenames must resolve to ready project sources.
 * Domain workflow packs are not used for import validation.
 */
const importedCaseSchema = z.object({
  label: z.string().trim().min(1).max(240),
  input: genericCaseInputSchema,
  expectedOutcome: genericCaseOutcomeSchema,
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    const [{ projectId }, access, body] = await Promise.all([
      context.params,
      context.params.then(({ projectId: id }) => authorizeProjectRequest(request, id, true)),
      request.json(),
    ]);
    ...
    const projectRows = await serviceRequest<Row[]>(`projects?id=eq.${encodeURIComponent(projectId)}&mode=eq.production&select=id&limit=1`);
    if (!projectRows[0]) throw new ApiError(404, 'Production project not found.');
    ...
    const extractionRows = await serviceRequest<Row[]>(
      `evidence_extractions?select=id,evidence_artifacts!inner(project_id,filename,display_name,status,scan_status)&evidence_artifacts.project_id=eq.${encodeURIComponent(projectId)}&evidence_artifacts.status=eq.ready&evidence_artifacts.scan_status=eq.clean`,
    );
    ...
    for (const extraction of extractionRows) {
      ...
      for (const name of [artifact.filename, artifact.display_name]) {
        ...
        const ids = sourceToExtractionIds.get(key) ?? [];
        ids.push(String(extraction.id));
        sourceToExtractionIds.set(key, ids);
      }
    }
    const cases = payload.cases.map((item, index) => {
      ...
      if (evidenceIds.length === 0) {
        throw new ApiError(
          400,
          `Case ${index + 1} does not reference a ready project evidence file. Upload the knowledge-transfer sources first, then use exact filenames in evidence_files.`,
        );
      }
      return {
        id: randomUUID(),
    ...
    await serviceRequest('audit_events', {
      method: 'POST',
      body: JSON.stringify([{
        organization_id: access.organizationId,
        project_id: projectId,
        actor_id: access.actor.id,
        event_type: 'evaluation.historical_cases_imported',
        payload: { count: cases.length, labels: cases.map((item) => item.label) },
      }]),
    });
    ...
    if (error instanceof z.ZodError) {
      ...
      const path = first?.path?.join('.') ?? 'case';
      return Response.json({
        error: `Case import is invalid at ${path}: ${first?.message ?? 'check label, input_json, expected_outcome_json (decision + reason), and evidence_files.'}`,
        recoverable: true,
      }, { status: 400 });
    }
