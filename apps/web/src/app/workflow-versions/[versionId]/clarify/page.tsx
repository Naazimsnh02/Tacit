...
export default function ClarificationPage({ params }: { readonly params: Promise<{ versionId: string }> }) {
  ...
  return <main><ClarificationInterview questions={questions} onAnswer={async (questionId, answer) => { const response = await fetch(`/api/questions/${questionId}/answer`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ answer }) }); const body = await response.json() as { error?: string }; if (!response.ok) throw new Error(body.error ?? 'Unable to apply the answer.'); setQuestions((current) => current.map((question) => question.id === questionId ? { ...question, status: 'answered', answer: String(answer), answerValue: answer as ClarificationQuestion['answerValue'], answeredAt: new Date().toISOString() } : question)); }} /></main>;
}
