...
export async function recordApprovalDecision(repository: ApprovalRepository, approvalId: string, rawInput: unknown) {
  ...
  const action = await repository.saveAction({ approvalRequestId: approvalId, decision: input.decision, comment: input.comment ?? null, approver: input.approver });
