...
export function ProductionObservation({ projectId }: { readonly projectId: string }) {
  ...
  return <WorkspaceShell active="Observe" mode="production" projectId={projectId} projectName={payload.project.name} versionId={payload.workflow.confirmedVersionId ?? undefined}>
    <PageHeader breadcrumb="Observe / production project" title={`Observe ${payload.workflow.name}`} description="Link manual steps, narration, transcript selections, and optional browser events to durable evidence." status={<StatusBadge status={canRecord ? 'observing' : 'draft'} />} actions={<a className="btn btn-secondary" href={`/projects/${projectId}/evidence`}>Back to evidence</a>} />
