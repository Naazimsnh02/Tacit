...
export function ObservationWorkspace({ projectId, workflowName, workspace, panelData, evidence }: ObservationWorkspaceProps) {
  ...
  return <WorkspaceShell active="Observe" projectId={projectId} projectName={workflowName}>
    <PageHeader breadcrumb={`Observe / ${workflowName}`} title="Watch an expert resolve a real exception" description="Tacit captures the evidence, judgment, and safety boundaries behind every decision." status={<StatusBadge status={status === 'recording' ? 'observing' : 'draft'} />} actions={<DemoControls stage="observe" onReset={resetDemo} onStart={startSession} />} />
    <section className="observation-hero">
      ...
      <div className="observation-hero-copy"><span className="observation-kicker"><span className="recording-dot" /> Live expert observation</span><h2>Invoice exception review</h2><p>Follow the evidence trail as Maya verifies a quantity mismatch before approving the invoice.</p><div className="observation-hero-meta"><span>INV-2048</span><span>Quantity mismatch</span><span>₹425,000</span></div></div>
    ...
    <section className="observation-stage">
      ...
      <aside className="tacit-rail">
        ...
        <div className="narration-card"><div><span className="eyebrow">Expert narration</span><button className="mic-button" type="button" disabled={!canRecord} onClick={captureMicrophone} aria-label="Use microphone">⌁</button></div><textarea value={narration} onChange={(event) => setNarration(event.target.value)} placeholder="Explain what makes this decision safe..." disabled={!canRecord} /><button className="text-action" type="button" disabled={!canRecord || !narration.trim()} onClick={saveNarration}>Add to evidence trail</button>{microphoneState === 'listening' && <p className="capture-feedback">Microphone ready — narration remains editable.</p>}{microphoneState === 'unavailable' && <p className="capture-feedback">Microphone unavailable — type your explanation instead.</p>}</div>
        <fieldset className="evidence-selector"><legend>Linked source material</legend>{evidence.map((item) => <label key={item.id}><input type="checkbox" checked={selectedEvidenceIds.includes(item.id)} onChange={() => setSelectedEvidenceIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /> <span>{item.title}</span></label>)}</fieldset>
      </aside>
    </section>
