...
export function ApprovalDashboard({ projectId }: { readonly projectId: string }) {
  ...
  async function load() { try { const response = await fetch(`/api/projects/${projectId}/approvals`, { headers: productionHeaders() }); const body = await response.json() as ApprovalRequest[] | { error: string }; if (!response.ok || !Array.isArray(body)) throw new Error(); setRequests(body); } catch { setError('The approval queue could not be loaded.'); } }
  useEffect(() => { void load(); }, [projectId]);
  async function decide(id: string, decision: typeof actions[number][0]) { setBusyId(id); setError(null); try { const response = await fetch(`/api/approvals/${id}/decision`, { method: 'POST', headers: productionHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ decision }) }); if (!response.ok) throw new Error(); await load(); } catch { setError('The approval decision was not recorded.'); } finally { setBusyId(null); } }
