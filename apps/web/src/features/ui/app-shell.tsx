...
export function WorkspaceShell({ children, active = 'Overview', projectId, projectName = 'Invoice Exception Review', versionId }: { readonly children: ReactNode; readonly active?: NavigationLabel; readonly projectId?: string; readonly projectName?: string; readonly versionId?: string }) {
  return <div className="app-shell"><aside className="sidebar"><a className="brand" href="/"><span className="brand-mark">T</span><span>Tacit</span></a><div className="project-context"><small>Active project</small><strong>{projectName}</strong></div><nav className="nav-list" aria-label="Project navigation">{labels.map((label, index) => <a className={`nav-link${label === active ? ' active' : ''}`} aria-current={label === active ? 'page' : undefined} href={hrefFor(label, projectId, versionId)} key={label}><span className="nav-index">{String(index + 1).padStart(2, '0')}</span>{label}</a>)}</nav><div className="sidebar-footer">Workflow intelligence platform</div></aside><main className="app-main"><div className="content">{children}</div></main></div>;
}
